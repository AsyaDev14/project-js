    ...
    <div class='product-buy-container'>
      ...
      <button class='btn-buy' type='button'>
        <svg width='18' height='18'>
          <use href='${e(n)}#icon-cart' />
        </svg>
      </button>
    </div>
      ${u?`<svg class='product-discount' width='60' height='60'>
      <use href='${e(n)}#icon-discount'/>
    </svg>`:""}
  </li>`}let g=document.querySelector(".filters-form-search"),h=new d;g.addEventListener("submit",function(t){t.preventDefault(),h.category=c.default.categorySelectEl.value,"Show All"===h.category&&(h.category="",console.log(h.category)),h.fetchProductsByQuery().then(t=>{let e=t.results.map(t=>f(t)).join("");c.default.productsListEl.innerHTML=e})}),h.fetchCategories().then(t=>{let e=t.map(t=>`<option value='${t}'>${t}</option>`).join("");(0,c.default).categorySelectEl.insertAdjacentHTML("beforeend",e),(0,c.default).categorySelectEl.insertAdjacentHTML("beforeend","<option value='Show All'>Show All</option>")});var c=i("4Nugj");function y(t){let{_id:a,name:r,img:s,category:i,size:o,popularity:c}=t;return`<li class="popular-item" data-product-id='${a}'>
      <div class="popular-product-img">
        <img src="${s}" alt="${r}" width="56" height="56" />
      </div>
      <button class="cart-btn" type="button">
        <svg width="12" height="12">
          <use href="${e(n)}#icon-cart"></use>
        </svg>
      </button>
      <div class="popular-item-info">
        ...
        <p class="popular-info-descr">
          <span class="popular-accent">Size:</span> ${o}
          <span class="popular-accent">Popularity:</span> ${c}
        </p>
      </div>
    </li>`}function b({_id:t,name:a,img:r,price:s}){return`
  <li class="discount-item" data-product-id='${t}'>
            <div class="discount-img-box">
                ...
                <svg class="discount-svg" width="60" height="60">
                    <use href="${e(n)}#icon-discount"></use>
                </svg>
            </div>
        ...
        </li>`}var v=i("i8Q71");let m=new d;async function w(){try{var t,e,a;let r;(0,v.updateCartOnHeader)(),t=(await m.fetchProductsByQuery()).results,c.default.productsListEl.innerHTML=t.map(f).join("");let s=await m.fetchPopular();c.default.popularListElement.innerHTML=s.sort(()=>Math.random()-.5).map(y).join(""),e=await m.fetchDiscount(),a=c.default.discountProductsEl,r=Math.ceil(7*Math.random()),a.innerHTML=e.map(b).slice(r,r+2).join("")}catch(t){console.log("err")}}window.addEventListener("DOMContentLoaded",w)}();
//# sourceMappingURL=index.ba1e0e32.js.map
